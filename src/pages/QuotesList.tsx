...
import {
  ...
  Clock, CheckCircle2, XCircle, ArrowRight, Plus, Send, AlertTriangle
} from "lucide-react";
...
const statusConfig: Record<string, { color: string; bg: string; icon: any; label: string; glow: string }> = {
  ...
  approved: { color: "text-emerald-400", bg: "bg-emerald-400/10", icon: CheckCircle2, label: "Approved", glow: "shadow-[0_0_20px_rgba(52,211,153,0.2)]" },
  rejected: { color: "text-red-400", bg: "bg-red-400/10", icon: XCircle, label: "Rejected", glow: "" },
  expired: { color: "text-amber-400", bg: "bg-amber-400/10", icon: AlertTriangle, label: "Expired", glow: "" },
};
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex items-center justify-between mb-12">
          <div>
            ...
            <p className="text-white/50 font-medium text-lg">
              Manage estimates and convert approved quotes to jobs
            </p>
          </div>
          <Button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
          <Card className="border-none glass rounded-[2rem] p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] uppercase font-black text-white/40 tracking-widest">Total Quotes</p>
          ...
          <Card className="border-none glass rounded-[2rem] p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] uppercase font-black text-white/40 tracking-widest">Pending Value</p>
                <p className="text-3xl font-black text-white">${pendingTotal.toFixed(2)}</p>
              </div>
            </div>
          </Card>
          <Card className="border-none glass rounded-[2rem] p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] uppercase font-black text-white/40 tracking-widest">Approved Value</p>
                <p className="text-3xl font-black text-white">${approvedTotal.toFixed(2)}</p>
              </div>
            </div>
          </Card>
        </div>
        ...
        {isLoading ? (
          ...
          <Card className="border-none glass p-16 text-center rounded-[3rem]">
            ...
            <h3 className="text-2xl font-black text-white mb-3">No Quotes Found</h3>
            <p className="text-white/50 mb-8">
              {searchQuery ? "Try adjusting your search criteria" : "Create your first quote to start estimating"}
            </p>
          </Card>
        ) : (
                    ...
                    <div className="flex items-start justify-between gap-6">
                      <div className="flex-1">
                        <div className="flex items-center gap-4 mb-4">
                          ...
                          <div className="text-white/30 font-mono text-sm">#{q.quoteNumber}</div>
                        </div>
                        <h3 className="text-xl font-black text-white mb-2 group-hover:text-primary transition-colors">
                          {q.customerName || `Quote #${q.quoteNumber}`}
                        </h3>
