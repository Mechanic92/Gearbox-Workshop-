...
--> statement-breakpoint
CREATE TABLE `agent_draft_quotes` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	...
	FOREIGN KEY (`taskId`) REFERENCES `agent_tasks`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
...
CREATE TABLE `checklist_items` (
	...
	`createdAt` integer DEFAULT (cast(strftime('%s', 'now') as integer)) NOT NULL,
	FOREIGN KEY (`templateId`) REFERENCES `checklist_templates`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE TABLE `jobs` (
	...
	`createdAt` integer DEFAULT (cast(strftime('%s', 'now') as integer)) NOT NULL,
	`updatedAt` integer DEFAULT (cast(strftime('%s', 'now') as integer)) NOT NULL,
	`agentStatus` text DEFAULT 'DISABLED',
	FOREIGN KEY (`ledgerId`) REFERENCES `ledgers`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`customerId`) REFERENCES `customers`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`vehicleId`) REFERENCES `vehicles`(`id`) ON UPDATE no action ON DELETE no action
);
...
CREATE TABLE `parts` (
	...
	FOREIGN KEY (`markupRuleId`) REFERENCES `markup_rules`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE `standard_price_list` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	`ledgerId` integer NOT NULL,
...
CREATE TABLE `vehicle_part_fitment` (
	...
	`sellPrice` real NOT NULL,
	`createdAt` integer DEFAULT (cast(strftime('%s', 'now') as integer)) NOT NULL,
	FOREIGN KEY (`specId`) REFERENCES `vehicle_specs`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE INDEX `accounting_integration_unique_idx` ON `accounting_integrations` (`ledgerId`,`provider`);--> statement-breakpoint
CREATE INDEX `accounting_sync_integration_idx` ON `accounting_sync_log` (`integrationId`);--> statement-breakpoint
CREATE INDEX `draft_quote_job_idx` ON `agent_draft_quotes` (`jobId`);--> statement-breakpoint
CREATE INDEX `draft_quote_task_idx` ON `agent_draft_quotes` (`taskId`);--> statement-breakpoint
CREATE INDEX `agent_task_job_idx` ON `agent_tasks` (`jobId`);--> statement-breakpoint
CREATE INDEX `agent_task_status_idx` ON `agent_tasks` (`status`);--> statement-breakpoint
CREATE INDEX `audit_ledger_idx` ON `audit_log` (`ledgerId`);--> statement-breakpoint
CREATE INDEX `audit_user_idx` ON `audit_log` (`userId`);--> statement-breakpoint
CREATE INDEX `aa_ledger_idx` ON `autonomous_actions` (`ledgerId`);--> statement-breakpoint
...
CREATE INDEX `fleet_ledger_idx` ON `fleets` (`ledgerId`);--> statement-breakpoint
...
CREATE INDEX `job_time_job_idx` ON `job_time_logs` (`jobId`);--> statement-breakpoint
CREATE INDEX `job_time_user_idx` ON `job_time_logs` (`userId`);--> statement-breakpoint
...
CREATE INDEX `part_markup_idx` ON `parts` (`markupRuleId`);--> statement-breakpoint
...
CREATE INDEX `std_price_ledger_idx` ON `standard_price_list` (`ledgerId`);--> statement-breakpoint
CREATE INDEX `std_price_name_idx` ON `standard_price_list` (`serviceName`);--> statement-breakpoint
...
CREATE INDEX `fitment_spec_idx` ON `vehicle_part_fitment` (`specId`);--> statement-breakpoint
CREATE INDEX `spec_make_model_idx` ON `vehicle_specs` (`make`,`model`);--> statement-breakpoint
